...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// coremap is the page frame array, defined in pagetable.h
		// Use swap_pageout if the frame has a dirty bit (modified, needs to write)
		if ((coremap[frame].pte->frame) & PG_DIRTY) {
			coremap[frame].pte->swap_off = swap_pageout(frame, coremap[frame].pte->swap_off);
			coremap[frame].pte->frame |= PG_ONSWAP; // Declared in pagetable.h
            evict_dirty_count++;
		} else {
            evict_clean_count++;
        }
		
		// The frame should also be invalid and unreferenced, set those bits to 0 (CSC258, use and not)
		//coremap[frame].pte->frame = coremap[frame].pte->frame | PG_ONSWAP;
		coremap[frame].pte->frame &= ~PG_VALID;
		coremap[frame].pte->frame &= ~PG_REF;
        //its brought back from swap so technically its not been modified
...
char *find_physpage(addr_t vaddr, char type) {
    ...
    //check pgdir[idx] is valid, if not initialize
    if(((pgdir[idx].pde) & PG_VALID)== 0){
        pgdir[idx] = init_second_level();
    }

    //get the pg table entry (2nd level)
    pgtbl_entry_t *ptable;
    ptable = (pgtbl_entry_t *) (pgdir[idx].pde & PAGE_MASK);

    //look for the index inside the table
    p = &(ptable[PGTBL_INDEX(vaddr)]); //gives the offset based on addr.
    ...
    if((p->frame & PG_VALID)) {
        ...
        miss_count++;
        //allocate frame for invalid p
        int frame = allocate_frame(p);
        //check swap valid
        if(p->frame & PG_ONSWAP){
            //p is in the swap, bring it in.
